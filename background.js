...
function checkTab(id, isBeforeNav, isRepeat) {
  ...
  function isSameHost(host1, host2) {
    return host1 == host2 || host1 == "www." + host2 || host2 == "www." + host1;
  }
      ...
      (async () => {
        try {
          ...
          if (!initialSites || initialSites.length === 0) {
            sendResponse({
              success: false,
              error: "AI could not generate an initial blocklist.",
            });
            return;
          }
          ...
          console.error("[LBNG Background] Error in startAiLockdown:", error);
          sendResponse({
            success: false,
      ...
      (async () => {
        try {
          ...
          if (
            ...
            !result.reason.includes("API error") &&
            ...
            console.log(
              "[LBNG Background] ✅ Cached analysis result for:",
              videoInfo.videoId
            );
          } else {
            console.log(
              "[LBNG Background] ⚠️ Not caching failed/incomplete analysis"
            );
          }
...
async function handleTabUpdated(tabId, changeInfo, tab) {
  ...
  if (
    ...
    if (
      isYouTube &&
      aiState.aiLockdownActive &&
      Date.now() < aiState.aiLockdownEndTime
    ) {
      console.log(
        "[LBNG Background] YouTube detected during AI lockdown - bypassing conventional blocking"
      );
...
function handleTick() {
  ...
  browser.storage.local.get(
    ["aiLockdownActive", "aiLockdownEndTime"],
    (data) => {
