...
function processBlockInfo(info) {
    ...
    if (themeLink) {
        themeLink.href =
            "/themes/" + (info.theme ? `${info.theme}.css` : "default.css");
    }
    ...
    if (info.unblockTime && unblockTime) {
        // Show exact time in local format
        try {
            const ts = Date.parse(info.unblockTime)
                ? new Date(info.unblockTime)
                : null;
            unblockTime.innerText = ts ? ts.toLocaleString() : info.unblockTime;
        } catch (e) {
...
// Inject a playful roast under the title area
function setRoast() {
    const roastEl = document.getElementById("lbRoast");
    if (!roastEl) return;
    const roasts = [
        "FocusHoot says: That tab wasn't going to help, and you know it.",
        "Owls don't get distracted. Be the owl.",
        "Plot twist: Productivity wins this round.",
        "Future you is nodding in approval right now.",
        "Another detour dodged. The goal stays on track.",
    ];
...
browser.runtime.sendMessage({ type: "blocked" }).then((info) => {
