...
const INTERNAL_BLOCK_URL =
  /^(\w+\/)?(blocked|delayed|password)\.html\?\$S&\$U$/;
...
const PER_SET_OPTIONS = {
  ...
  // Inside the GENERAL_OPTIONS object in common.js
  ...
  days: {
    type: "array",
    def: [false, true, true, true, true, true, false],
    id: "day",
  },
...
const GENERAL_OPTIONS = {
  // def: default value, id: form element identifier (see options.html)
  isAiEnabled: { id: "isAiEnabled", type: "boolean", def: false },
  apiKey: { id: "apiKey", type: "string", def: "" },
  aiBlockSet: { id: "aiBlockSet", type: "string", def: "1" },
...
function checkTimePeriodsFormat(times) {
  return (
    times == "" ||
    /^[0-2]\d[0-5]\d-[0-2]\d[0-5]\d([, ]+[0-2]\d[0-5]\d-[0-2]\d[0-5]\d)*$/.test(
      times
    )
  );
}
